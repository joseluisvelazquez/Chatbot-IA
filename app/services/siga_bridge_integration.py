...
_BRIDGE_ENRICHMENT_TIMEOUT_SECONDS = 6.0
...
async def _named_safe_bridge_call(
    key: str,
    label: str,
    action: Callable[[], Awaitable[dict[str, Any] | list[Any] | None]],
    *,
    ...
    data, error = await _safe_bridge_call(label, action, company_id=company_id)
    return key, data, error


async def _collect_bridge_enrichment(
    calls: list[tuple[str, str, Callable[[], Awaitable[dict[str, Any] | list[Any] | None]]]],
    *,
    company_id: int | None,
) -> tuple[dict[str, dict[str, Any] | list[Any] | None], list[str]]:
    if not calls:
        return {}, []
    ...
    try:
        results = await asyncio.wait_for(
            asyncio.gather(
                *[
                    _named_safe_bridge_call(
                        key,
                        label,
                        action,
                        company_id=company_id,
                    )
                    for key, label, action in calls
                ]
            ),
            timeout=_BRIDGE_ENRICHMENT_TIMEOUT_SECONDS,
        )
    except asyncio.TimeoutError:
        logger.warning(
            "siga_bridge_verification_enrichment_timeout",
            extra={
                "company_id": company_id,
                "timeout_seconds": _BRIDGE_ENRICHMENT_TIMEOUT_SECONDS,
                "calls": [label for _key, label, _action in calls],
            },
        )
        return {}, [SigaBridgeUnavailableError.__name__]

    data_by_key: dict[str, dict[str, Any] | list[Any] | None] = {}
    errors: list[str] = []
...
async def enrich_verification_with_bridge(
    ...
    *,
    ...
    data_by_key, errors = await _collect_bridge_enrichment(calls, company_id=company_id)
    bridge["customer"] = data_by_key.get("customer")
    bridge["account"] = data_by_key.get("account")
    bridge["payments"] = data_by_key.get("payments")
