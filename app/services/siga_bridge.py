...
_CACHE_MISS = object()
_inflight: dict[tuple[str, tuple[tuple[str, str], ...]], asyncio.Task[Any]] = {}
_inflight_lock = asyncio.Lock()
_shared_http_client: httpx.AsyncClient | None = None
_shared_http_client_lock = asyncio.Lock()
_shared_bridge_client: "SigaBridgeClient | None" = None
...
def _record_contract_error() -> None:
    _metrics["contract_errors"] += 1


def _record_non_json_error() -> None:
    _metrics["non_json_errors"] += 1


def _record_logical_error() -> None:
    _metrics["logical_errors"] += 1
...
def reset_siga_bridge_metrics() -> None:
    ...
    _metrics["timeouts"] = 0
    _metrics["contract_errors"] = 0
    _metrics["non_json_errors"] = 0
    _metrics["logical_errors"] = 0
...
def get_siga_bridge_metrics() -> dict[str, Any]:
    ...
    return {
        ...
        "contract_errors": int(_metrics["contract_errors"]),
        "non_json_errors": int(_metrics["non_json_errors"]),
        "logical_errors": int(_metrics["logical_errors"]),
...
class SigaBridgeClient:
    ...
    def _validate_config(self) -> None:
        ...
        allow_insecure_local = bool(
            ...
            or getattr(settings, "SIGA_BRIDGE_ALLOW_INSECURE_LOCAL", False)
        )

        if scheme not in {"http", "https"} or not hostname:
            raise SigaBridgeConfigError("SIGA Bridge base URL is invalid")

        if allow_insecure_local:
            return

        if scheme != "https":
            raise SigaBridgeConfigError("SIGA Bridge base URL must use HTTPS outside local debug")

        if hostname in {"localhost", "127.0.0.1", "::1"} or hostname.endswith(".local"):
            raise SigaBridgeConfigError("SIGA Bridge base URL cannot point to localhost in production")
    ...
    async def _http_client(self) -> tuple[httpx.AsyncClient, bool]:
        ...
        async with _shared_http_client_lock:
            ...
            return _shared_http_client, False
    ...
    async def _get(
        ...
        *,
        ...
        cacheable = _CACHE_TTLS_SECONDS.get(action) is not None
        if cacheable and not bypass_cache:
            key = _cache_key(action, request_params)
            async with _inflight_lock:
                active_task = _inflight.get(key)
                if active_task is None:
                    active_task = asyncio.create_task(
                        self._fetch_uncached(action, request_params)
                    )
                    _inflight[key] = active_task
                    active_task.add_done_callback(
                        lambda task, cache_key=key: _schedule_inflight_cleanup(cache_key, task)
                    )
                else:
                    logger.info("siga_bridge_inflight_hit", extra={"action": action})

            data = await asyncio.shield(active_task)
            return copy.deepcopy(data)

        return await self._fetch_uncached(action, request_params)
    ...
    async def _fetch_uncached(
        ...
        for attempt in range(1, attempts + 1):
            ...
            try:
                _metrics["requests"] += 1
                response = await self._request_once(request_params)
    ...
    async def _request_once(self, request_params: dict[str, Any]) -> httpx.Response:
        client, should_close = await self._http_client()
        try:
    ...
    def _handle_response(
        ...
        try:
            ...
            _record_contract_error()
            raise

        if not ok:
            _record_logical_error()
...
def get_siga_bridge_client() -> SigaBridgeClient:
    global _shared_bridge_client
    if _shared_bridge_client is None:
        _shared_bridge_client = SigaBridgeClient()
    return _shared_bridge_client


async def close_siga_bridge_client() -> None:
    global _shared_http_client
