...
def test_ok_false_with_2xx_raises_contract_error():
    ...
    assert get_siga_bridge_metrics()["logical_errors"] == 1
...
def test_html_2xx_raises_contract_error_with_body_preview():
    ...
    assert get_siga_bridge_metrics()["non_json_errors"] == 1
...
def test_inflight_deduplicates_concurrent_customer_requests():
    calls = {"count": 0}
    ...
    async def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            ...
            json=bridge_payload(
                action="customer",
                data={"customers": [{"nombre": "Cliente Concurrente"}]},
            ),
        )
    ...
    async def scenario():
        return await asyncio.gather(
            client.get_customer_by_phone("4421234567", company_id=1),
            client.get_customer_by_phone("4421234567", company_id=1),
            client.get_customer_by_phone("4421234567", company_id=1),
        )
    ...
    assert results == [
        {"customers": [{"nombre": "Cliente Concurrente"}]},
        {"customers": [{"nombre": "Cliente Concurrente"}]},
        {"customers": [{"nombre": "Cliente Concurrente"}]},
    ]
    assert calls["count"] == 1
